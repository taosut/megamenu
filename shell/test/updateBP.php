...
class Ved_Create_Products  extends Mage_Shell_Abstract
{
    ...
    public function createProducts(){
        ...
        $query = 'select `id`, `code`, `category`, `name`, `label`, `manufacture`, `supplier`, `supplier_code`, 
				`warranty`, `supplier_price`, `price`, `type`, `connection`, `cable`, `db`, `tech`, `led`, `colour`, `short_description`, `image1`
                        from banphim ';
        $products    = $connection->fetchAll($query);
        print "Start update " . count($products) . " products.\n";
        $update = 0;
        $non_update_test = 0;
        $non_update = 0;
        $already = 0;
        foreach($products as $prod){
            try{
               ...
               if ($attr->usesSource()) {
                   $ledId = $attr->getSource()->getOptionId($prod['led']);
               }
                ...
                if ($attr->usesSource()) {
                    $connectionId = $attr->getSource()->getOptionId($prod['connection']);
                }
                if($prod['image1']){
...
//                    ->setStoreId(1) //you can set data in store scope
                    ...
                    ->setAttributeSetId(25) //ID of a attribute set named 'default'
...
//                    ->setCountryOfManufacture('AF') //country of manufacture (2-letter country code)
 ...
 //                   ->setMetaDescription('test meta description 2')
                 ...
                 //   ->setCategoryIds(array(2, 10)); //assign product to categories
                if(!$prod['id']){
