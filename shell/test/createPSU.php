...
class Ved_Create_Products  extends Mage_Shell_Abstract
{
    ...
    public function createProducts(){
        ...
        $query = 'select `ID`, `code`, `category`, `name`, `label`, `manufacture`, `supplier`, `supplier_code`, `warranty`, `supplier_price`, 
				`price`, `cong_suat_psu`, `quat_he_thong_psu`, `tuoi_tho_psu`, `kich_thuoc_psu`, `input_psu`, `out_psu`, `cong_ket_noi_psu`, `cong_nghe_psu`, 
        ...
        foreach($products as $prod){
            try{
               ...
               $attr = $product->getResource()->getAttribute("cong_suat_psu");
               if ($attr->usesSource()) {
                   $keyboardTypeId = $attr->getSource()->getOptionId(trim($prod['cong_suat_psu']));
               }
...
//                    ->setStoreId(1) //you can set data in store scope
                    ...
                    ->setAttributeSetId(30) //ID of a attribute set named 'default'
...
//                    ->setCountryOfManufacture('AF') //country of manufacture (2-letter country code)
 ...
 //                   ->setMetaDescription('test meta description 2')
                    ...
                    ->setData('cong_suat_psu',$keyboardTypeId) // ket noi
                    //->setData('chuotquang',$chuotquangId)
                                        //->setCable($prod['cable'])
                    //->setDpi($prod['dpi'])
                    ->setData('quat_he_thong_psu',$prod['quat_he_thong_psu'])
										->setData('tuoi_tho_psu',$prod['tuoi_tho_psu'])
										->setData('kich_thuoc_psu',$prod['kich_thuoc_psu'])
										->setData('input_psu',$prod['input_psu'])
										->setData('out_psu',$prod['out_psu'])
										->setData('cong_ket_noi_psu',$prod['cong_ket_noi_psu'])
										->setData('cong_nghe_psu',$prod['cong_nghe_psu'])
                    ...
                    ->setCategoryIds(array(2,5,63)); //assign product to categories
                if(!$prod['id']){
