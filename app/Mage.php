...
final class Mage
{
     ...
     * @var null
     */
    static private $_messageQueueConnection = null;
    ...
    /**
     * Singleton instant connection
     * @return null|\PhpAmqpLib\Connection\AMQPStreamConnection
     */
    public static function getInstantMessageQueueConnection()
    {
        $config = (array)self::getConfig()->getNode('global/rabitmq');
     ...
     * @throws Exception
     */
    public static function callMessageQueue(array $data, $routing_key)
    {
        try {
            Mage::getModel('teko_amp/queue')->addData([
            ...
            Mage::log($e->getMessage(), null, 'push_order_message_queue.log');
            throw new Exception("Không đẩy được thông tin sang kho");
        }
    }

    public static function pushMessageQueue($data, $routing_key, $properties = [], $exchange = 'teko.sale')
    {
        try {
            $connection = self::getInstantMessageQueueConnection();
            $channel = $connection->channel();
            $channel->exchange_declare($exchange, 'topic', false, true, false);
            $message = new PhpAmqpLib\Message\AMQPMessage($data, $properties);
            $channel->basic_publish($message, $exchange, $routing_key);
            $channel->close();
            $connection->close();
            self::saveLogMessageQueue(1, $exchange, $routing_key, $data, $properties);

        } catch (Exception $e) {
            Mage::log($e->getTraceAsString(),null, 'push_message_queue_log.log');
            throw new Exception("khong day duoc thong tin len mesage queue");
        }
    }

    public static function receiveMessageQueue($callback)
    {
        try {
            $connection = self::getInstantMessageQueueConnection();
            $channel = $connection->channel();
            echo ' [*] Waiting for logs. To exit press CTRL+C', "\n";
            $channel->basic_consume('teko.sale', '', false, false, false, false, $callback);
    ...
    /**
     * @param int $type : 1: producer, 2: consumer
     * @param $exchange
     * @param $routingKey
     * @param string $messageBody : Default blank
     * @param array $properties
     * @throws Exception
     */
    public static function saveLogMessageQueue($type = 1, $exchange, $routingKey, $messageBody = '', $properties = [])
    {
        $time = time();

        try {
            $write = self::getSingleton('core/resource')->getConnection('core_write');
