...
class Mage_Adminhtml_IndexController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function logoutAction()
    {
        ...
        //logout
		$apiSiteSso = (string)Mage::getConfig()->getNode('global/sso_site');
		$authenMethod= Mage::getModel('core/variable')->loadByCode('authen_method')->getValue('text');

		if($authenMethod == 0 || $authenMethod == 1){
			$this->_redirect('*');		
		} elseif ($authenMethod == 2){
			$this->_redirectUrl($apiSiteSso. 'signout?_cont=http://teksvr.com');
		} else {
            if (isset($_COOKIE['_uat'])) { unset($_COOKIE['_uat']); }
            setcookie('_uat', '', time() - 1000000, '/', '.teksvr.com', false, false);

			$url = Mage::getBaseUrl();
			$this->_redirectUrl($apiSiteSso. 'signin?_cont='.$url);
		}
    }
