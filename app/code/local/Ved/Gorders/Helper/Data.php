...
class Ved_Gorders_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->apiUrlWarehouse = (string)Mage::getConfig()->getNode('global/warehouse_api_url').'v2/';
        if ($adapter == 'phongvu') {
            $this->apiUrlWarehouse = (string)Mage::getConfig()->getNode('global/phongvu_api_url');
        }
    ...
    protected static $_preStatus = array('Mới tạo', 'Đủ hàng', 'Không đủ hàng');

    public function getPreStatusName($status)
    {
        return self::$_preStatus[$status];
    }

    private function getPurchaseTransfer(Mage_Sales_Model_Order $order, Mage_Sales_Model_Order_Item $item)
    {
        $data = Mage::getModel('ved_gorders/transfer')
            ->getCollection()
            ->addFieldToFilter('order_id', $order->getId())
            ->addFieldToFilter('order_item_id', $item->getItemId())
            ->addFieldToFilter('status', array('in' => array(1, 2)))
    ...
    private function hasPurchaseMQ(Mage_Sales_Model_Order $order, Mage_Sales_Model_Order_Item $item)
    {
        $data = Mage::getModel('ved_gorders/purchaserequestitem')
            ->getCollection()
            ->addFieldToFilter('order_id', $order->getId())
            ->addFieldToFilter('order_item_id', $item->getItemId())
            ->addFieldToFilter('status', array('in' => array(1, 2)))
            ->addFieldToFilter('purchase_id', array('notnull' => true));
        return $data->count();
    }
    ...
    public function getPurchaseHtml(Mage_Sales_Model_Order $order, Mage_Sales_Model_Order_Item $item)
    {
        ...
        if ($this->_getAssignedState($order->getStatus()) == Mage_Sales_Model_Order::STATE_NEW) {
            ...
            $adapter = Mage::getConfig()->getNode('global/warehouse_adapter');
            if ($adapter == 'phongvu') {
                $html .= "<button class='button view-warehouse' data-sku='".$item->getWarehouseSku()."'>Thông tin tồn kho</button>";
            }
            else {
                if (!$this->hasPurchaseMQ($order, $item)) {
                    $html .= "<a href='$url'><button class='button'>Purchase Warehouse</button></a>";
                }
            }
        }
        return $html;
    ...
    {
        if ($path == 'stores/list') {
            return explode('v2', $this->apiUrlWarehouse)[0] . $path;
        }
    ...
    {
        $purchases = Mage::getModel('ved_gorders/orderitemstock')
            ->getCollection()
            ->addFieldToFilter('status', 1)
        ...
        if (count($purchases)) {
            ...
            $client = new Varien_Http_Client(Mage::helper('ved_gorders')->getApiUrlWarehouse('products/unhold'));
    ...
    public function isPVEnv()
    {
        return Mage::getConfig()->getNode('global/warehouse_adapter') == 'phongvu';
    }
}