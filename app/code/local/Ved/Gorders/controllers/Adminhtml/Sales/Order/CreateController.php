 ...
 */
class Ved_Gorders_Adminhtml_Sales_Order_CreateController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $order_old = $this->_getSession()->getOrder();
            $order = $this->_getOrderCreateModel()
                ->setIsValidate(true)
                ->importPostData($this->getRequest()->getPost('order'))
                ->createOrder();
            $order_id = $order_old->getId();
            /**
             * @var Mage_Sales_Model_Order $order_old
             */
            $order_old->callMessageQueueCancel("-1");
            $order_old->cancelPurchaseAndStock();
            if ($this->_getSession()->getReordered()) {
                $order->addStatusHistoryComment('<b>' . $userAdmin . '</b> đặt lại hàng<br/>');
            } else {
                if ($order_id) {
                    $order->addStatusHistoryComment('<b>' . $userAdmin . '</b> sửa đơn hàng<br/>');
                } else {
                    $order->addStatusHistoryComment('<b>' . $userAdmin . '</b> tạo mới đơn hàng<br/>');
                }
            }
            if ($order_old->getTotalPaid() && $order_old->getPayment()->getMethod() == 'purchaseorder')
                $order->setTotalPaid($order_old->getTotalPaid());
            // Save Affilicate code
            if ($order_old->getAffiliateCode())
                $order->setAffiliateCode($order_old->getAffiliateCode());

            if ($order_old->getDepositAmount()) {
                $order->setDepositAmount($order_old->getDepositAmount());
                $order->setDepositMethod($order_old->getDepositMethod());
                $order->setDepositDescription($order_old->getDepositDescription());
            }
            $order->setCustomerNote($order_old->getCustomerNote());
            $order->save();
            $this->_getSession()->clear();
