 ...
 */
class Ved_Gorders_Adminhtml_Sales_Order_EditController extends Ved_Gorders_Adminhtml_Sales_Order_CreateController
{
    /**
    ...
    {
        $orderId = $this->_getSession()->get('order_id')['order_id'];

        $order = Mage::getModel('sales/order')->load($orderId);
        if ($order->getData('is_instalment')) {
            Mage::getSingleton('adminhtml/session')->addError("Đơn hàng trả góp không được phép sửa.");
