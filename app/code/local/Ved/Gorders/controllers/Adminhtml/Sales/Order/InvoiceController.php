 ...
 */
class Ved_Gorders_Adminhtml_Sales_Order_InvoiceController extends Mage_Adminhtml_Controller_Sales_Invoice
{
    /**
    ...
    public function startAction()
    {
        ...
        $check = false;
        $d1 = new DateTime('2017-05-15 00:00:01');
        ...
        if(!$check && $orderDate > $d1){
            try{
                throw new Exception('Đơn hàng chưa được xác nhận với người mua. Vui lòng lựa chọn "Đã xác nhận" trước Gửi đơn hoặc tạo hóa đơn!');
            }catch(Exception $e){
                $this->_getSession()->addException($e, $this->__('Đơn hàng chưa được xác nhận với người mua. Vui lòng lựa chọn "Đã xác nhận" trước Gửi đơn hoặc tạo hóa đơn!'));
                Mage::logException($e);
        ...
        try {
            ...
            if ($invoice) {
                ...
                $order->setCustomerNoteNotify(!empty($data['send_email']))
                    ->setIsInProcess(true)
                    ->setActionFlag($order::ACTION_FLAG_INVOICE, true);
                if ($order->getAddressesCollection($order::ACTION_FLAG_SHIP) === true) {
                    $order->setActionFlag($order::ACTION_FLAG_SHIP, false);
                }
