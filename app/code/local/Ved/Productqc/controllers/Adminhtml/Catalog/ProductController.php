 ...
 */
class Ved_Productqc_Adminhtml_Catalog_ProductController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    protected function _initProduct()
    {
        ...
        /**
         * @comment Register old product
         * @by DungBV
         */
        Mage::register('old_product_sku', $product->getData('warehouse_sku'));
...
//            }
    ...
    protected function _initProductSave()
    {
         ...
         */
        if ($product->getId() && isset($productData['url_key_create_redirect'])) // && $product->getOrigData('url_key') != $product->getData('url_key')
        {
...
//                }
                ...
                $productJsonData = Mage::helper('core')->jsonEncode($product->getData());
                $admin_user_session = Mage::getSingleton('admin/session');
                $adminuserId = $admin_user_session->getUser()->getUserId();
                $logAction = Mage::getModel("ved_productqc/logaction");
                $logAction->setUserId($adminuserId)
                    ->setAction('UPDATE')
                    ->setObjectId($productId)
                    ->setObjectData($productJsonData)
                    ->setObject('PRODUCT')
    ...
    public function duplicateAction()
    {
        ...
        try {
            ...
            $newProduct->setData('warehouse_sku', null);
            $newProduct->setData('sku', null);
    ...
    {
        if (Mage::getSingleton('admin/session')->isAllowed('admin/catalog/delete_product') && $id = $this->getRequest()->getParam('id')) {
            $product = Mage::getModel('catalog/product')
                ->load($id);
            $name = $product->getName();
            try {
                $product->delete();
                $this->_getSession()->addSuccess($this->__("The product $name has been deleted."));
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            }
        }
