...
class Ved_SalesRule_Adminhtml_Promo_QuoteController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        if ($this->getRequest()->getPost()) {
            try {
                ...
                // Nam: Write to log table
                if ($model->getId())
                {
                    $promoJsonData = Mage::helper('core')->jsonEncode($model->getData());
                    ...
                    $logAction->setUserId($adminuserId)
                        ->setAction('UPDATE')
                        ->setObjectId($model->getId())
                        ->setObjectData($promoJsonData)
                        ->setObject('PROMO_SHOPPING_CART')
                        ->setCreatedAt(date("Y-m-d H:i:s",time()))
                        ->save();
                }

                //End write to log table
