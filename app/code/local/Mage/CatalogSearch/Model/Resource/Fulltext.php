...
class Mage_CatalogSearch_Model_Resource_Fulltext extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    private function queryResult($adapter, $searchType, $query, $searchWord)
    {
        $preparedTerms = Mage::getResourceHelper('catalogsearch')
            ->prepareTerms($searchWord, $query->getMaxQueryWords());

        $bind = array();
        $likeCond = '';
        $likeQuery = "s.data_index LIKE '";
        if ($searchType == Mage_CatalogSearch_Model_Fulltext::SEARCH_TYPE_LIKE
            || $searchType == Mage_CatalogSearch_Model_Fulltext::SEARCH_TYPE_COMBINE
        ) {
            $words = Mage::helper('core/string')->splitWords($searchWord, true, $query->getMaxQueryWords());
    ...
    public function prepareResult($object, $queryText, $query)
    {
        ...
        if (!$query->getIsProcessed()) {
            ...
            $this->queryResult($adapter, $searchType, $query, $queryText);

            $keyword = Mage::helper('catalog/product_url')->format(strtolower($queryText));
            $this->queryResult($adapter, $searchType, $query, $keyword);
            ...
            foreach ($synonyms as $synonym) {
                ...
                if ($index !== null) {
                    foreach ($synonym as $key => $value) {
                        ...
                        if ($index !== $key) {
                            $this->queryResult($adapter, $searchType, $query, $searchWord);
                        }
                    }
                    break;
                }
