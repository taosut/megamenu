...
//        return $result;
    ...
    public function getPrice()
    {
        if ($this->getTypeId() == 'bundle' &&
            $this->getPriceType() == Mage_Bundle_Model_Product_Price::PRICE_TYPE_DYNAMIC
        ) {
            $price = 0;
            $bundleCollection = $this->getTypeInstance(true)
                ->getSelectionsCollection($this->getTypeInstance(true)->getOptionsIds($this), $this);
    ...
    /** Get product full url (with breadcrumb) for SEO **/
    public function getProductFullUrl()
    {
        $deepestCategory = Mage::getResourceModel('catalog/category_flat_collection')
            ->addIdFilter($this->getCategoryIds())
            ->addAttributeToFilter('is_active', 1)
            ->setOrder('level', 'DESC')
            ->getFirstItem();

        if (isset($deepestCategory)) {
            $urlPath = $this->getUrlPath(Mage::getModel('catalog/category')->load($deepestCategory->getId()));
            $urlKey = (strpos($urlPath, '.html') !== false) ? '' : $this->getUrlKey() . '.html';
            $productUrl = Mage::getUrl('') . $urlPath . $urlKey;
        } else {
