...
class Mage_Catalog_Block_Product_View extends Mage_Catalog_Block_Product_Abstract
{
    ...
    protected function _prepareLayout()
    {
        ...
        if ($headBlock) {
            ...
            if ($description) {
                ...
                $headBlock->setDescription(Mage::helper('core/string')->substr(Mage::getStoreConfig('design/head/default_description'), 0));
            }
    ...
    public function getRelatedItems()
    {
        ...
        $fromPrice = $_product->getFinalPrice() * 0.9;
        $toPrice = $_product->getFinalPrice() * 1.1;
        $relatedProductsCollection = Mage::getResourceModel('catalog/product_collection')
            ->setStoreId(Mage::app()->getStore()->getId())
            ->setVisibility(Mage::getSingleton('catalog/product_visibility')->getVisibleInCatalogIds())
            ->addAttributeToSelect(Mage::getSingleton('catalog/config')->getProductAttributes())
            ->addFieldToFilter('price', array('from' => $fromPrice, 'to' => $toPrice))
            ->addFieldToFilter('entity_id', array('neq' => $_product->getId()))
            ->addAttributeToFilter('attribute_set_id', $attributeSetId)
            ->addAttributeToFilter('instock', array('in' => array(1, 6)))
            ->setCurPage(10);
        return $relatedProductsCollection;
    }
}
