...
class Mage_Adminhtml_Block_Sales_Order_View_Info extends Mage_Adminhtml_Block_Sales_Order_Abstract
{
    ...
    /**
     * Get link to edit affiliate code
     *
     * @param Mage_Sales_Model_Order_Address $address
     * @param string $label
     * @return string
     */
    public function getAffiliateCodeEditLink($label = '')
    {
        if (empty($label)) {
            $label = $this->__('Edit');
        }
        $url = $this->getUrl('*/sales_order/affiliate', array('order_id' => $this->getOrder()->getId()));
        if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/affiliate'))
            return '<a href="' . $url . '">' . $label . '</a>';
        else
            return "";
    }

    public function getDepositEditLink()
    {
        if (empty($label)) {
            $label = $this->__('Edit');
        }
        $url = $this->getUrl('*/sales_order/deposit', array('order_id' => $this->getOrder()->getId()));
        if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/deposit') && $this->getOrder()->allowUpdateDeposit())
            return '<a href="' . $url . '">' . $label . '</a>';
        else
            return "";
    }

    public function getVatEditLink()
    {
        return $this->getUrl('*/sales_order/vat', array('order_id' => $this->getOrder()->getId()));
    }
