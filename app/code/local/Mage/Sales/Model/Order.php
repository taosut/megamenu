 ...
 * @method int getIsSendQueue()
 * @method $this  setIsSendQueue(bool $value)
 * @method $this  setDepositAmount(float $value)
 * @method float  getDepositAmount()
 * @method float  getTotalCodAmount()
 * @method $this  setTotalCodAmount(float $value)
 * @method $this  setDepositMethod(string $value)
 * @method string  getDepositMethod()
 * @method $this setEstimateDelivery(string $estimateDelivery)
 * @method string getDepositDescription()
 * @category    Mage
 * @package     Mage_Sales
...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    {
        if ($this->getState() != "processing") return false;
        if ($this->canUnhold() || $this->isPaymentReview()) {
            return false;
        }
        $state = $this->getState();
    ...
    {
        $userAdmin = Mage::getSingleton('admin/session')->getUser()->getUsername();
        $message = '<b>' . $userAdmin . '</b> giải phóng đơn hàng<br/>';
        if (!$this->canUnhold()) {
            Mage::throwException(Mage::helper('sales')->__('Unhold action is not available.'));
        }
        $this->setState($this->getHoldBeforeState(), $this->getHoldBeforeStatus(), $message);
    ...
    {
        $userAdmin = Mage::getSingleton('admin/session')->getUser()->getUsername();
        $message = '<b>' . $userAdmin . '</b> hủy đơn hàng<br/>';
        if ($this->canCancel()) {
            $this->getPayment()->cancel();
            $this->registerCancellation($message);
    ...
    protected function _checkState()
    {
        ...
        if ($userObj) {
            $userAdmin = $userObj->getUsername();

            if ($this->getActionFlag(self::ACTION_FLAG_SHIP) === true) {
                $comment = '<b>' . $userAdmin . '</b> tạo phiếu vận chuyển<br/>';
            } elseif ($this->getActionFlag(self::ACTION_FLAG_INVOICE) === true) {
                $comment = '<b>' . $userAdmin . '</b> tạo hóa đơn<br/>';
            }
        }
        ...
        if (!$this->isCanceled()
            ...
            if (0 == $this->getBaseGrandTotal() || $this->canCreditmemo()) {
                if ($this->getState() !== self::STATE_COMPLETE) {
                    $this->_setState(self::STATE_COMPLETE, true, $comment, $userNotification);
                }
            } /**
        ...
        if ($this->getState() == self::STATE_NEW && $this->getIsInProcess()) {
            $this->setState(self::STATE_PROCESSING, true, $comment, $userNotification);
        }
        return $this;
    }
    ...
    //Add new state for order
    public function canVerify()
    {
        $state = $this->getState();
        $status = $this->getStatus();
        if ($state === self::STATE_NEW && $status === "pending") {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->canVerify()) {
            $this->setState(Mage_Sales_Model_Order::STATE_NEW, "telephone_confirm");
        }
        return $this;
    }
    ...
    public function canReady()
    {
        $state = $this->getState();
        $status = $this->getStatus();
        if ($state === self::STATE_NEW && $status === "telephone_confirm") {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->canReady()) {
            $this->setState(Mage_Sales_Model_Order::STATE_NEW, "good_ready");
        }
        return $this;
    }
    ...
    {
        if ($this->canReject()) {
            $this->_setState(Mage_Sales_Model_Order::STATE_COMPLETE, "reject");
        }
        return $this;
    }

    //Redelivery order
    public function canRedelivery()
    {
        $state = $this->getState();
        $status = $this->getStatus();
    ...
    {
        if ($this->canRedelivery()) {
            $this->setState(Mage_Sales_Model_Order::STATE_PROCESSING, "redelivery");
        }
        return $this;
    }
     ...
     * @return string
     */
    private function getProductTypeCombo($item)
    {
     ...
     * @return bool
     */
    private function isOrderItemCombo(Mage_Sales_Model_Order_Item $item)
    {
        return $item->getProductType() == 'bundle';
    }


    private function fillItem(Mage_Sales_Model_Order_Item $item)
    {
        $product = $item->getProduct();
        if ($item->getProductType() == 'simple' &&
            (!$item->getStandardProductId() || !$item->getWarehouseSku())
        )
            throw new Exception('Sản phẩm:' . $item->getProduct()->getName() . 'mã kho không chính xác');
        $result = [
            "invoiceItemId" => $item->getId(),
            "comboId" => $item->getParentItemId(),
            "productId" => $item->getStandardProductId(),
            "productType" => $this->getProductTypeCombo($item),
            "sku" => $item->getWarehouseSku(),
            "quantity" => $item->getQtyOrdered(),
            "price" => (string)(int)($item->getPrice() - $item->getDiscountAmount() / $item->getQtyOrdered()) . '.0000',
            "discountPercent" => $item->getDiscountPercent(),
            "discountAmount" => $item->getDiscountAmount(),
            "rowTotal" => (string)(int)(($item->getPrice() - $item->getDiscountAmount() / $item->getQtyOrdered()) * $item->getQtyOrdered()) . '.0000',
            "unitPrice" => $item->getPrice(),
            "warranty" => $item->getProduct()->getWarranty() ?
                (string)(int)$item->getProduct()->getWarranty() : "",
            "warehouseId" => $this->getWarehouseIdFromOrderItem($item),
            "note" => ""
        ];
        if ($this->isOrderItemCombo($item)) {
            $result['productId'] = $item->getProduct()->getId();
            $result['sku'] = $item->getProduct()->getSku();
            $result['comboName'] = $item->getName();
        }
        return $result;
    }
     ...
     * @throws Exception
     */
    public function getDataMessageQueue()
    {
        $data = array();
        ...
        foreach ($this->getAllVisibleItems() as $item) {
            ...
            switch ($item->getProductType()) {
                case 'simple':
                    $data[] = $this->fillItem($item);
                    break;
                case 'configurable':
                    $this->fillConfigurableItem($data, $item);
                    break;
                case 'bundle':
                    $data[] = $this->fillItem($item);
                    $this->fillComboItem($data, $item);
                    break;
                default:
                    break;
            }
        ...
        // Get admin comment
        $latestComment = $this->getStatusHistoryCollection()->getLastItem()->getComment();
        $explodeComment = explode('<br />', $latestComment, 2);
        $adminComment = '';
        if (isset($explodeComment[1]) && !empty($explodeComment[1])) {
            if (!empty($this->getCustomerNote()) && substr($this->getCustomerNote(), -1) != '.') {
                $adminComment = '.';
            }
            $adminComment .= "\n" . strip_tags($latestComment);
        }
        Mage::dispatchEvent('sale_order_send_queue_before', ['order' => $this]);
        return array(
            "id" => $this->getId(),
            "storeId" => $this->getStore()->getId(),
            "storeName" => $this->getStore()->getName(),
            "grandTotal" => $this->getGrandTotal(),
            "source" => $this->getSourceMessageQueue(),
            "payments" => $this->getPaymentsMessageQueue(),
            "paidAmount" => $this->getPaidAmount(),
            "unpaidAmount" => $this->getUnpaidAmount(),
            "unpaidMethod" => "COD",
            "discountRuleName" => $this->getDiscountDescription() ? $this->getDiscountDescription() : "",
            "discountDescription" => "",
            "customerId" => $this->getCustomerId(),
            "customerName" => $this->getCustomerName(),
            "customerNote" => $this->getCustomerNote() . $adminComment,
            "orderIdReplaced" => $this->getRelationParentId(),
            "orderCode" => $this->getIncrementId(),
            "shipDate" => $this->getEstimateDeliveryTimestamp(),
            "createdAt" => DateTime::createFromFormat('Y-m-d H:i:s', $this->getCreatedAt())->getTimestamp(),
            "userActionInfo" => $this->getUserActionInfor(),
            "addresses" => array(
                'shipping' => array(
                    "addressCode" => $this->getShippingAddressCode(),
                    "name" => $this->getShippingAddress()->getName(),
                    "province" => $this->getShippingAddress()->getRegion(),
                    "district" => $this->getShippingAddress()->getCity(),
                    "street" => $this->getShippingAddress()->getStreet1(),
                    "email" => $this->getShippingAddress()->getEmail(),
                    "telephone" => $this->getShippingAddress()->getTelephone()
                ),
                'billing' => array(
                    "addressCode" => $this->getBillingAddressCode(),
                    "name" => $this->getBillingAddress()->getName(),
                    "province" => $this->getBillingAddress()->getRegion(),
                    "district" => $this->getBillingAddress()->getCity(),
                    "street" => $this->getBillingAddress()->getStreet1(),
                    "email" => $this->getBillingAddress()->getEmail(),
                    "telephone" => $this->getBillingAddress()->getTelephone(),
                    "company" => "",
                    "MaSoThue" => "",
                    "sendTo" => $this->getBillingAddress()->getStreet(),
                )
            ),
            'items' => $data,
        );
     ...
     * @param Mage_Sales_Model_Order_Item $item
     */
    private function fillComboItem(&$items, Mage_Sales_Model_Order_Item $item)
    {
        if ($this->isOrderItemCombo($item)) {
            foreach ($item->getChildrenItems() as $item) {
                 ...
                 * @var  Mage_Sales_Model_Order_Item $item
                 */
                $items[] = $this->fillItem($item);
            }
        }
    }

    private function fillConfigurableItem(&$items, Mage_Sales_Model_Order_Item $item)
    {
        foreach ($item->getChildrenItems() as $childrenItem) {
            $items[] = array_merge($this->fillItem($item), [
                "invoiceItemId" => $childrenItem->getId(),
                "comboId" => null,
                "productId" => $childrenItem->getStandardProductId(),
                "productType" => $this->getProductTypeCombo($childrenItem),
                "sku" => $childrenItem->getWarehouseSku(),
                "quantity" => $item->getQtyOrdered(),
                "price" => $item->getPrice(),
                "discountPercent" => $item->getDiscountPercent(),
                "discountAmount" => $item->getDiscountAmount(),
                "rowTotal" => $item->getRowTotal(),
                "warehouseId" => $this->getWarehouseIdFromOrderItem($childrenItem),
                "note" => ""
            ]);
        }
    }
     ...
     * @return  int|null
     */
    private function getWarehouseIdFromOrderItem($orderItem)
    {
        $stock = Mage::getModel('ved_gorders/orderitemstock')->getCollection()
            ->addFieldToFilter('order_id', $orderItem->getOrderId())
            ->addFieldToFilter('order_item_id', $orderItem->getId())
            ->getFirstItem();
        if ($stock->getStoreId()) return $stock->getStoreId();
        return Mage::getModel('ved_gorders/purchaserequestitem')->getCollection()
            ->addFieldToFilter('order_id', $orderItem->getOrderId())
            ->addFieldToFilter('order_item_id', $orderItem->getId())
            ->getFirstItem()->getStoreId();
    }
     ...
     * @return bool
     */
    public function isExistHoldItem($storeId, $product_id)
    {
        /**
         * @var Ved_Gorders_Model_Orderitemstock
         */
        $model = Mage::getModel('ved_gorders/orderitemstock');
        $data = $model->getCollection()
            ->addFieldToFilter('order_id', $this->getId())
            ->addFieldToFilter('status', 1)
            ->addFieldToFilter('store_id', array('neq' => $storeId))
            ->addFieldToFilter('product_id', array('neq' => $product_id))
            ->getFirstItem();
        return !($data->getId());
    }

    public function notSendQueue()
    {
        return !$this->getIsSendQueue();
    }

    /**
     * @return null|string
     */
    private function getShippingAddressCode()
    {
        $shippingAddress = $this->getShippingAddress();
        $city = Mage::getModel('teko_amp/city')->getCollection()
            ->addFieldToFilter('region_id', ['in' => $this->getRegionIds($shippingAddress->getRegionId())])
            ->addFieldToFilter('name', ['like' => '%' . $shippingAddress->getCity() . '%'])
            ->getFirstItem();
        return $city->getCode() ?
            $city->getCode() :
            $this->getProvinceCode($shippingAddress->getRegionId());
    }

    /**
     * @return null|string
     */
    private function getBillingAddressCode()
    {
        $billingAddress = $this->getBillingAddress();
        $city = Mage::getModel('teko_amp/city')->getCollection()
            ->addFieldToFilter('region_id', array('in' => $this->getRegionIds($billingAddress->getRegionId())))
            ->addFieldToFilter('name', ['like' => '%' . $billingAddress->getCity() . '%'])
            ->getFirstItem();
        return $city->getCode() ?
            $city->getCode() :
            $this->getProvinceCode($billingAddress->getRegionId());
    }
     ...
     * @return array
     */
    private function getRegionIds($regionId)
    {
        $region = Mage::getModel('teko_amp/region')
            ->getCollection()
            ->addFieldToFilter('region_id', $regionId)
            ->getFirstItem();
        $regions = Mage::getModel('teko_amp/region')
            ->getCollection()
            ->addFieldToFilter('province_code', $region->getProvinceCode())
            ->load();
        $ids = [];
     ...
     * @return string
     */
    private function getProvinceCode($regionId)
    {
        return Mage::getModel('teko_amp/region')
            ->getCollection()
            ->addFieldToFilter('region_id', $regionId)
            ->getFirstItem()
            ->getProvinceCode();
    ...
    public function allowUpdateDeposit()
    {
        return !$this->getIsSendQueue();
    }
     ...
     * @param string $reason
     */
    public function callMessageQueueCancel($reason = "")
    {
        return;
    }
     ...
     * @param string $comment
     */
    public function insertHistory($time, $comment)
    {
     ...
     * @throws Exception
     */
    public function updateDepositAmount($deposit)
    {
        if ($this->allowUpdateDeposit()) {
            $this->setDepositAmount($deposit)->save();
            return;
        }
        throw new Exception('Order not allow update deposit');
    }
    ...
    {
        if ($this->getData('estimate_delivery')) {
            $date = DateTime::createFromFormat('Y-m-d H:i:s', $this->getData('estimate_delivery'), new DateTimeZone('UTC'));
            return $date->setTimezone(new DateTimeZone('Asia/Ho_Chi_Minh'))->format('d/m/Y H:i:s');
        }
        return null;
    }
     ...
     * @return null|string
     */
    public function getEstimateDeliveryTimestamp()
    {
        if ($this->getData('estimate_delivery')) {
            $date = DateTime::createFromFormat('Y-m-d H:i:s', $this->getData('estimate_delivery'), new DateTimeZone('UTC'));
     ...
     * @return bool
     */
    public function refundToGoogleAnalytics()
    {
        $analytics = new TheIconic\Tracking\GoogleAnalytics\Analytics(true);
        $analytics->setProtocolVersion('1')
            ->setClientId('555')
            ->setTrackingId(Mage::getModel('core/variable')->loadByCode('ga_track_id')->getValue('html'))
            ->setAffiliation('Admin Magento')
            ->setRevenue($this->getGrandTotal() * (-1));
        foreach ($this->getAllVisibleItems() as $item)
            /**
             * @var Mage_Sales_Model_Order_Item $item
             */
            $analytics->addProduct([
                ...
                'name' => $item->getName(),
                'price' => $item->getPrice(),
                'quantity' => (int)$item->getQtyOrdered() * (-1),
            ]);
        $analytics->setProductActionToPurchase();
        $analytics->setEventCategory('Checkout')
            ->setEventAction('Purchase')
            ->sendEvent();
        return $analytics->sendEvent()->getHttpStatusCode() == 200;
    }

    protected function createToGoogleAnalytics()
    {
        $analytics = new TheIconic\Tracking\GoogleAnalytics\Analytics(true);
        $analytics->setProtocolVersion('1')
            ->setClientId('555')
            ->setTrackingId(Mage::getModel('core/variable')->loadByCode('ga_track_id')->getValue('html'))
            ->setAffiliation('Admin Magento')
            ->setRevenue($this->getGrandTotal())
            ->setTax($this->getTaxAmount())
            ->setShipping($this->getShippingAmount())
    ...
    {
        $this->addStatusHistoryComment('Khách hàng đã thanh toán Online', 'telephone_confirm')
            ->setIsVisibleOnFront(false)
            ->setIsCustomerNotified(false);
        Mage::callMessageQueue($this->getDataMessageQueue(), 'sale.order.create');
        $this->setIsSendQueue(true);
        $this->save();
    }
    ...
    {
        return Mage::getModel('TEKShop_PaymentOnline/transaction')->getCollection()
            ->addFieldToFilter('order_id', $this->getId())
            ->getFirstItem()
            ->getAmount();
    }
     ...
     *
     */
    public function cancelPurchaseAndStock()
    {
    ...
    public function initShipment()
    {
        ...
        $shipment = Mage::getModel('sales/service_order', $this)->prepareShipment($savedQtys);
    ...
    private function _getItemQtys()
    {
        ...
        foreach ($this->getAllVisibleItems() as $item) {
             ...
             * @var Mage_Sales_Model_Order_Item $item
             */
            $result[$item->getId()] = $item->getQtyOrdered();
        }
        return $result;
    }

    public function getDepositMethodLabel()
    {
        if (isset($this->_deposit[$this->getDepositMethod()]))
            return $this->_deposit[$this->getDepositMethod()];
        return '';
    }

    public function getDepositLabels()
    {
        return $this->_deposit;
    }

    public function allowEditVat()
    {
        return !($this->getStatus() == 'complete' || $this->getStatus() == 'canceled');
    }

    /**
     *
     */
    private function getSourceMessageQueue()
    {
        if ($this->getData('affiliate_code')) {
            return [
                "channel" => 'TELESALES',
                "terminalId" => "",
                "agentId" => trim($this->getData('affiliate_code')),

            ];
        }
        if ($this->getOriginalIncrementId())
            $originalIncrementId = $this->getOriginalIncrementId();
        else
            $originalIncrementId = $this->getIncrementId();
        $source = Mage::getModel('ved_gorders/order_source')
            ->getCollection()
            ->addFieldToFilter('original_increment_id', $originalIncrementId)
            ->getFirstItem();

        return [
            "channel" => $source->getData('channel'),
            "terminalId" => $source->getData('terminal_id'),
            "agentId" => $source->getData('agent_id') ? $source->getData('agent_id') : "",

        ];
    }

    private function getPaymentsMessageQueue()
    {
        $result = [];
        if ($this->getData('is_instalment') == '1') {
            /**
             * @var Ved_Instalment_Model_Order_Instalment $orderInstalment
             */
            ...
            $result[] = [
                "paidMethod" => "INSTALMENT",
                "transactionData" => [
                    "id" => $orderInstalment->getData('id'),
                    "amount" => (int)$orderInstalment->getData('amount') -
                        (int)$orderInstalment->getData('fee') -
                        (int)$orderInstalment->getData('prepaid_amount'),
                    "partner" => $orderInstalment->getData('provider'),
                    "details" => "Thanh toán trả góp theo hồ sơ " . $orderInstalment->getData('transaction_id')
                ]
            ];
            if ($orderInstalment->getData('fee'))
                $result[] = [
                    "paidMethod" => "INSTALMENT_FEE",
                    "transactionData" => [
                        "id" => $orderInstalment->getData('id'),
                        "amount" => (int)$orderInstalment->getData('fee'),
                        "partner" => $orderInstalment->getData('provider'),
                        "details" => $orderInstalment->getData('description')
                    ]
                ];
            if ($orderInstalment->getData('prepaid_amount') && $orderInstalment->getData('prepaid_method'))
                $result[] = [
                    "paidMethod" => $orderInstalment->getData('prepaid_method'),
                    "transactionData" => [
                        "id" => $orderInstalment->getData('id'),
                        "amount" => (int)$orderInstalment->getData('prepaid_amount'),
                        "partner" => $orderInstalment->getData('provider'),
                        "details" => "Tra truoc tra gop theo ho so " . $orderInstalment->getData('transaction_id')
                    ]
                ];
        }
        $this->getPaymentOnlineMethod($result);
        $this->getPaymentDeposit($result);
        return $result;
    }

    public function getInstalmentAmount()
    {
        return (float)Mage::getModel('ved_instalment/order_instalment')
            ->getCollection()
            ->addFieldToFilter('order_id', $this->getId())
            ->addFieldToFilter('status', true)
            ->getFirstItem()
    ...
    {
        if ($this->getData('affiliate_code')) {
            $createdBy = Mage::getModel('teko_sso/admin_sso')->load(trim($this->getData('affiliate_code')), 'asia_id');
            if ($createdBy->getId())
                return [
                    "id" => $createdBy->getData('sso_id'),
                    "asiaId" => $createdBy->getData('asia_id'),
                    "type" => "online_sales"
                ];
            else
                return [
                    "id" => trim($this->getData('affiliate_code')),
                    "asiaId" => trim($this->getData('affiliate_code')),
                    "type" => "online_sales"
                ];
        }
        $source = Mage::getModel('ved_gorders/order_source')->load(
            $this->getOriginalIncrementId() ? $this->getOriginalIncrementId() : $this->getIncrementId(),
            'original_increment_id'
        );
        if ($source->getData('agent_id')) {
            $createdBy = Mage::getModel('teko_sso/admin_sso')->load($source->getData('agent_id'), 'sso_id');
            if ($createdBy) {
                if ($createdBy->getId())
                    return [
                        "id" => $createdBy->getData('sso_id'),
                        "asiaId" => $createdBy->getData('asia_id'),
                        "type" => $source->getData('channel') == "APP" ? "offline_sales" : "operator"
                    ];
            }
        }
        return [
    ...
    private function getPaymentDeposit(&$result)
    {
        if ((bool)$this->getData('deposit_amount') && (bool)$this->getData('deposit_method'))
            $result[] = [
                "paidMethod" => $this->getData('deposit_method'),
                "transactionData" => [
                    "id" => (int)$this->getId(),
                    "amount" => (int)$this->getData('deposit_amount'),
                    "partner" => null,
                    "details" => $this->getData('deposit_description'),
                ]
            ];
    }

    private function getPaymentOnlineMethod(&$result)
    {
        $transaction = Mage::getModel('TEKShop_PaymentOnline/transaction')->getCollection()
            ->addFieldToFilter('order_id', $this->getId())
            ->getFirstItem();
        if ($transaction->getId()) {
            $result[] = [
                "paidMethod" => "ONLINE",
                "transactionData" => [
                    "id" => $this->getPayment()->getPoNumber(),
                    "amount" => (int)$transaction->getData('amount'),
                    "partner" => "VnPay",
                    "details" => "Thanh toán theo hóa đơn " . $this->getQuoteId(),
                ]
            ];
        }
    }
