...
;(function ( $j, window, document, undefined ) {
    ...
    $j.extend(Plugin.prototype, {
        ...
        updateOriginalInput:function(){
            var newValue = "";
            $j('.pincode-input-text',this._container).each(function( index, value ) {
                newValue += $j(value).val().toString();
            });
            $j(this.element).val(newValue);
        },
        check: function(){
            var isComplete = true;
            var code = "";
            $j('.pincode-input-text',this._container).each(function( index, value ) {
                code += $j(value).val().toString();
                if(!$j(value).val()){
                    isComplete = false;
                }
            });
        ...
        buildInputBoxes: function () {
            this._container = $j('<div />').addClass('pincode-input-container');
            ...
            if( this.settings.hidedigits == false && $j(this.element).val() !=""){
                currentValue = $j(this.element).val().split("");
            }

            // make sure this is the first password field here
            if(this.settings.hidedigits){
                this._pwcontainer = $j('<div />').css("display", "none").appendTo(this._container);
                this._pwfield = $j('<input>').attr({'type':'password','pattern': "[0-9]*", 'inputmode':"numeric",'id':'preventautofill','autocomplete':'off'}).appendTo(this._pwcontainer);
            }

            if(this._isTouchDevice()){
                // set main class
                $j(this._container).addClass("touch");
                ...
                var wrapper = $j('<div />').addClass('touchwrapper touch'+this.settings.inputs).appendTo(this._container);
                var input = $j('<input>').attr({'type':'number','pattern': "[0-9]*", 'placeholder':touchplaceholders, 'inputmode':"numeric",'maxlength':this.settings.inputs,'autocomplete':'off'}).addClass('form-control pincode-input-text').appendTo(wrapper);

                var touchtable = $j('<table>').addClass('touchtable').appendTo(wrapper);
                var row = $j('<tr/>').appendTo(touchtable);
                ...
                for (var i = 0; i <  this.settings.inputs; i++) {
                    if(i == (this.settings.inputs-1)){
                        $j('<td/>').addClass('last').appendTo(row);
                    }else{
                        $j('<td/>').appendTo(row);
                    }
                }
                ...
                for (var i = 0; i <  this.settings.inputs; i++) {
                    ...
                    var input = $j('<input>').attr({'type':'text','maxlength':"1",'autocomplete':'off','placeholder':(placeholders[i] ? placeholders[i] : undefined)}).addClass('form-control pincode-input-text').appendTo(this._container);
                    if(this.settings.hidedigits){
                        // hide digits
                        input.attr('type','password');
                    }else{
            ...
            // error box
            this._error = $j('<div />').addClass('text-danger pincode-input-error').appendTo(this._container);

            //hide original element and place this before it
            $j(this.element).css( "display", "none" );
            this._container.insertBefore(this.element);
        },
        enable:function(){
            $j('.pincode-input-text',this._container).each(function( index, value ) {
                $j(value).prop('disabled', false);
            });
        },
        disable:function(){
            $j('.pincode-input-text',this._container).each(function( index, value ) {
        ...
        focus:function(){
            $j('.pincode-input-text',this._container).first().select().focus();
        },
        clear:function(){
            $j('.pincode-input-text',this._container).each(function( index, value ) {
                $j(value).val("");
        ...
        _addEventsToInput:function(input,inputnumber){
            ...
            input.on('keydown', $j.proxy(function(e){
                if(this._pwfield){
                    ...
                    $j(this._pwfield).attr({'type':'text'});
                    $j(this._pwfield).val("");
                }
                ...
                if(this._isTouchDevice()){
                    if(e.keyCode  == 8 || e.keyCode  == 46){
                        ...
                        if($j(this.element).val().length == this.settings.inputs){
                            e.preventDefault();
                            e.stopPropagation();
                        }
                    }
            ...
            input.on('keyup', $j.proxy(function(e){
                ...
                // oncomplete check
                if(this.check()){
                    this.settings.complete($j(this.element).val(), e, this._error);
                }

                //onchange event for each input
                if(this.settings.change){
                    this.settings.change(e.currentTarget,$j(e.currentTarget).val(),inputnumber);
                }
